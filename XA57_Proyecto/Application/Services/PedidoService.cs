...
namespace XA57_Proyecto.Application.Services
{
    public class PedidoService : IPedidoService
    {
        ...
        public async Task<PedidoResultDto> AgregarAsync(CarritoItemDto item)
        {
            ...
            _logger.LogInformation("Pedido {PedidoId} creado: Producto={ProductoId}, Cantidad={Cantidad}, Modelo={ModeloId}, Linea={LineaId}",
                pedido.Id, item.ProductoId, item.Cantidad, item.ModeloAutobusId, item.LineaId);
        ...
        public async Task ActualizarEstadoAsync(int id, string estado)
        {
            ...
            {
                _logger.LogWarning("Intento de transición inválida: Pedido {PedidoId} de '{EstadoActual}' a '{EstadoNuevo}'",
                    id, pedido.Estado, estado);
            ...
            _logger.LogInformation("Pedido {PedidoId}: estado actualizado '{EstadoAnterior}' -> '{EstadoNuevo}'",
                id, pedido.Estado, estado);
        }
